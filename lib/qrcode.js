...
qrcode.decode = function (canvas_qr, debug_context) {
	...
	return qrcode.process(debug_context)
}
...
qrcode.process = function (debug_context) {
	let image = qrcode.grayScaleToBitmap(qrcode.grayscale())

	if (debug_context) {
		const imgData = debug_context.createImageData(globals.width, globals.height)
		...
		debug_context.putImageData(imgData, 0, 0)
	}
